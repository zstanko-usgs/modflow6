...
If no release settings are specified, then a single particle will be released from each release point at the beginning of the simulation.  If release times are configured via period block settings, the release setting for a given period block will continue to apply for subsequent stress periods until overridden by release settings for a later period block.  The example input file below demonstrates how to explicitly specify a single release at the beginning of the first stress period, equivalent to the default release behavior.
