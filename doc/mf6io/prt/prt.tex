...
A cell for which IDOMAIN <= 0 is considered not to exist in the simulation. A particle terminates without being released when the release point is in a cell for which IDOMAIN <= 0.
...
A cell for which IDOMAIN = 0 is considered not to exist in the simulation. When a particle encounters a cell face that is shared with a cell for which IDOMAIN = 0, that cell face is treated as an outer boundary of the model.

A cell for which IDOMAIN = -1 is a vertical pass-through cell that is considered not to exist in the simulation. Such a cell effectively acts as a direct hydraulic connection that allows particles to pass from the cell above to the cell below.
...
\item \texttt{8}: particle terminated immediately upon attempted release into an inactive cell
...
\item \texttt{6}: particle dropped to water table
\end{description}
